#![doc(html_root_url = "https://docs.rs/bilrost-build/0.12.1")]
#![allow(clippy::option_as_ref_deref, clippy::format_push_string)]

//! `bilrost-build` compiles `.proto` files into Rust.
//!
//! `bilrost-build` is designed to be used for build-time code generation as part of a Cargo
//! build-script.
//!
//! ## Example
//!
...
//! ```
//!
//! First, add `bilrost-build`, `bilrost` and its public dependencies to `Cargo.toml`
//! (see [crates.io](https://crates.io/crates/bilrost) for the current versions):
//!
//! ```toml
//! [dependencies]
//! bytes = <bytes-version>
//! bilrost = <bilrost-version>
//!
//! [build-dependencies]
//! bilrost-build = { version = <bilrost-version> }
//! ```
//!
...
//! ```
//!
//! To generate Rust code from `items.proto`, we use `bilrost-build` in the crate's
//! `build.rs` build-script:
//!
//! ```rust,no_run
//! use std::io::Result;
//! fn main() -> Result<()> {
//!     bilrost_build::compile_protos(&["src/items.proto"], &["src/"])?;
//!     Ok(())
//! }
//! ```
//!
...
//! of the `bilrost-build` crate and when your code is generated, the code docs will automatically
//! be cleaned up a bit.
//!
//! ## Sourcing `protoc`
//!
//! `bilrost-build` depends on the Protocol Buffers compiler, `protoc`, to parse `.proto` files into
//! a representation that can be transformed into Rust. If set, `bilrost-build` uses the `PROTOC`
...
//! ```
//!
//! If no `PROTOC` environment variable is set then `bilrost-build` will search the
//! current path for `protoc` or `protoc.exe`. If `bilrost-build` can not find `protoc`
...
//!
//! // Now compile your proto files via bilrost-build
//! ```
//!
...
use bilrost::Message;
use bilrost_types::{FileDescriptorProto, FileDescriptorSet};
...
///
/// Such framework isn't part of Bilrost at present.
...
impl Config {
    ...
    /// ```rust
    /// # let mut config = bilrost_build::Config::new();
    /// // Bilrost renames fields named `in` to `in_`. But if serialized through serde,
    /// // they should as `in`.
    ...
    /// Configures the code generator to not use the `bilrost_types` crate for Protobuf well-known
    ...
    ///
    /// `extern_path` allows `bilrost` types in external crates to be referenced in generated code.
    ///
    /// When `bilrost` compiles a `.proto` which includes an import of another `.proto`, it will
    ...
    /// # Example
    ///
    /// As an example, consider a crate, `uuid`, with a `bilrost`-generated `Uuid` type:
    ///
    /// ```proto
    /// // uuid.proto
    ///
    ...
    /// ```
    ///
    /// A separate crate, `my_application`, uses `bilrost` to generate message types which reference
    /// `Uuid`:
    ///
    /// ```proto
    ...
    /// ```
    ///
    /// Without configuring `uuid` as an external path in `my_application`'s `build.rs`, `bilrost`
    ...
    /// call to `.extern_path(".uuid", "::uuid")`, `bilrost` will use the external type instead of
    ...
    /// the `bilrost-types` crate for implementing reflection capabilities, among other things.
    ///
    /// ## Example
    ///
    /// In `build.rs`:
    ...
    /// let file_descriptor_set = bilrost_types::FileDescriptorSet::decode(&file_descriptor_set_bytes[..]).unwrap();
    /// ```
    ...
    /// descriptor set as an input file, rather than having bilrost-build generate the file by calling
    /// protoc.
    ///
    /// In `build.rs`:
    ///
    ...
    /// This style is non-idiomatic in Rust, so by default `bilrost` strips the enum name prefix from
    /// variants which include it. Configuring this option prevents `bilrost` from stripping the
    /// prefix.
    ...
    /// This is mainly useful for generating crates that wish to re-export bilrost.
    /// Defaults to `::bilrost::Message` if not specified.
    pub fn bilrost_path<S>(&mut self, path: S) -> &mut Self
    where
        S: Into<String>,
    {
        self.bilrost_path = Some(path.into());
        self
    }
    ...
    /// # use std::io::Result;
    /// fn main() -> Result<()> {
    ///   let mut bilrost_build = bilrost_build::Config::new();
    ///   // Enable a protoc experimental feature.
    ///   bilrost_build.protoc_arg("--experimental_allow_proto3_optional");
    ///   bilrost_build.compile_protos(&["src/frontend.proto", "src/backend.proto"], &["src"])?;
    ///   Ok(())
    /// }
    /// ```
    ...
    ///
    /// ```rust,no_run
    /// # use bilrost_types::FileDescriptorSet;
    ...
    ///
    /// This method is like the `bilrost_build::compile_protos` function, with the added ability to
    ...
    pub fn compile_protos(
        ...
        let file_descriptor_set_path = if let Some(path) = &self.file_descriptor_set_path {
            ...
            tmp = tempfile::Builder::new().prefix("bilrost-build").tempdir()?;
            tmp.path().join("bilrost-descriptor-set")
        };
        ...
        if !self.skip_protoc_run {
            ...
            let output = cmd.output().map_err(|error| {
                Error::new(
                    error.kind(),
                    format!("failed to invoke protoc (hint: https://docs.rs/bilrost-build/#sourcing-protoc): (path: {:?}): {}", &protoc, error),
                )
            })?;
    ...
    /// code contents.
    ///
    /// This is generally used when control over the output should not be managed by Bilrost,
    ...
    pub fn generate(
        ...
        let extern_paths = ExternPaths::new(&self.extern_paths, self.bilrost_types)
            .map_err(|error| Error::new(ErrorKind::InvalidInput, error))?;
...
impl fmt::Debug for Config {
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt.debug_struct("Config")
            ...
            .field("bilrost_types", &self.bilrost_types)
            ...
            .field("bilrost_path", &self.bilrost_path)
            .finish()
    }
}
...
///   let file_descriptor_set = fds();
///
///   bilrost_build::compile_fds(file_descriptor_set)
/// }
/// ```
...
For more information: https://docs.rs/bilrost-build/#sourcing-protoc
",
