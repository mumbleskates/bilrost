...
use bilrost_types::field_descriptor_proto::{Label, Type};
use bilrost_types::source_code_info::Location;
use bilrost_types::{
...
impl<'a> CodeGenerator<'a> {
    ...
    fn append_message(&mut self, message: DescriptorProto) {
        ...
        self.buf.push_str(&format!(
            ...
            self.config.bilrost_path.as_deref().unwrap_or("::bilrost")
        ));
    ...
    fn append_skip_debug(&mut self, fq_message_name: &str) {
        if self.should_skip_debug(fq_message_name) {
            push_indent(self.buf, self.depth);
            self.buf.push_str("#[bilrost(skip_debug)]");
            self.buf.push('\n');
        }
    }
    ...
    fn append_field(&mut self, fq_message_name: &str, field: FieldDescriptorProto) {
        ...
        let bilrost_path = self.config.bilrost_path.as_deref().unwrap_or("::bilrost");

        if repeated {
            self.buf
                .push_str(&format!("{}::alloc::vec::Vec<", bilrost_path));
        } else if optional {
            self.buf.push_str("::core::option::Option<");
        }
        if boxed {
            self.buf
                .push_str(&format!("{}::alloc::boxed::Box<", bilrost_path));
        }
    ...
    fn resolve_type(&self, field: &FieldDescriptorProto, fq_message_name: &str) -> String {
        ...
        match field.r#type() {
            ...
            Type::String => format!("{}::alloc::string::String", bilrost_path),
            Type::Bytes => self
                .config
                .bytes_type
                .get_first_field(fq_message_name, field.name())
...
impl MapType {
    /// The `bilrost-derive` annotation type corresponding to the map type.
    ...
    fn rust_type(&self) -> &'static str {
        match self {
            ...
            MapType::BTreeMap => "::bilrost::alloc::collections::BTreeMap",
        }
    }
}

impl BytesType {
    /// The `bilrost-derive` annotation type corresponding to the bytes type.
    fn annotation(&self) -> &'static str {
        match self {
            BytesType::Vec => "vec",
    ...
    fn rust_type(&self) -> &'static str {
        match self {
            BytesType::Vec => "::bilrost::alloc::vec::Vec<u8>",
            BytesType::Bytes => "::bilrost::bytes::Bytes",
        }
    }
}
