...
impl MapTy {
    ...
    fn lib(&self) -> TokenStream {
        match self {
            ...
            MapTy::BTreeMap => quote! { bilrost::alloc },
        }
    }
}
...
impl Field {
    ...
    pub fn encode(&self, ident: TokenStream) -> TokenStream {
        ...
        let ke = quote!(::bilrost::encoding::#key_mod::encode);
        let kl = quote!(::bilrost::encoding::#key_mod::encoded_len);
        ...
        match &self.value_ty {
            ...
            ValueTy::Scalar(value_ty) => {
                ...
                let ve = quote!(::bilrost::encoding::#val_mod::encode);
                let vl = quote!(::bilrost::encoding::#val_mod::encoded_len);
                quote! {
                    ::bilrost::encoding::#module::encode(
                        #ke,
                        #kl,
                        #ve,
                        #vl,
            ...
            ValueTy::Message => quote! {
                ::bilrost::encoding::#module::encode(
                    #ke,
                    #kl,
                    ::bilrost::encoding::message::encode,
    ...
    pub fn merge(&self, ident: TokenStream) -> TokenStream {
        ...
        match &self.value_ty {
            ...
            ValueTy::Scalar(value_ty) => {
                let val_mod = value_ty.module();
                let vm = quote!(::bilrost::encoding::#val_mod::merge);
                quote!(::bilrost::encoding::#module::merge(#km, #vm, &mut #ident, buf, ctx))
            }
            ValueTy::Message => quote! {
                ::bilrost::encoding::#module::merge(
                    #km,
                    ::bilrost::encoding::message::merge,
                    &mut #ident,
                    buf,
                    ctx,
                )
